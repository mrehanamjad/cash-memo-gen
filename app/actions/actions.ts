...
type EmailPayload = {
  customerName: string;
  customerEmail: string;
  itemGroups: ItemGroup[]; // Changed from Item[] to ItemGroup[]
  receiptId: string;
  subTotal: number;
  discount: number;
  tax: number;
...
export async function sendReceiptEmail(data: EmailPayload) {
  try {
    ...
    // --- GENERATE TABLE ROWS ---
    // We map through groups, printing the main item, then any engravings underneath it
    const itemsHtml = itemGroups.map((group) => {
      // 1. Main Item Row
      const mainRow = `
        <tr>
          <td class="row-item" style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; font-weight: 500;">
            ${group.mainItem.description}
          </td>
          <td class="row-item" style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; text-align: center;">
            ${group.mainItem.qty}
          </td>
          <td class="row-item" style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; text-align: right;">
            Rs. ${group.mainItem.price.toLocaleString()}
          </td>
          <td class="row-total" style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; text-align: right; font-weight: bold;">
            Rs. ${(group.mainItem.qty * group.mainItem.price).toLocaleString()}
          </td>
        </tr>
      `;

      // 2. Engraving Rows (if any)
      const engravingRows = group.engravings.map((eng) => `
        <tr>
          <td class="row-item" style="padding: 6px 0 6px 20px; border-bottom: 1px solid #f3f4f6; font-size: 13px; color: #6b7280;">
            + ${eng.description}
          </td>
          <td class="row-item" style="padding: 6px 0; border-bottom: 1px solid #f3f4f6; text-align: center; font-size: 13px; color: #6b7280;">
            ${eng.qty}
          </td>
          <td class="row-item" style="padding: 6px 0; border-bottom: 1px solid #f3f4f6; text-align: right; font-size: 13px; color: #6b7280;">
            Rs. ${eng.price.toLocaleString()}
          </td>
          <td class="row-total" style="padding: 6px 0; border-bottom: 1px solid #f3f4f6; text-align: right; font-size: 13px; color: #6b7280;">
            Rs. ${(eng.qty * eng.price).toLocaleString()}
          </td>
        </tr>
      `).join('');

      return mainRow + engravingRows;
    }).join('');
    ...
    const emailHtml = `
      ...
      <head>
        ...
        <style>
          ...
          .header-bg { background-color: #7f1d1d; }
          .header-text { color: #ffffff; }
          .header-sub { color: #fecaca; }
          .text-main { color: #1f2937; }
          .text-muted { color: #6b7280; }
      ...
      <body>
        <div class="container">
          ...
          <div style="background-color: #ffffff; padding: 40px 30px;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f9fafb; border: 1px solid #e5e7eb; padding: 20px; border-radius: 12px; margin-bottom: 30px;">
              <tr>
                <td width="50%" valign="top">
                  <div class="table-header" style="border: none; padding: 0; margin-bottom: 5px;">Receipt #</div>
                  <div class="text-main" style="font-size: 16px; font-weight: 700;">${receiptId}</div>
            ...
            <table width="100%" style="margin-top: 20px;">
              <tr>
                <td class="text-muted" style="text-align: right; padding: 3px 0; font-size: 14px;">Sub Total:</td>
                <td class="text-main" style="text-align: right; font-weight: 600; padding: 3px 0; width: 120px; font-size: 14px;">Rs. ${subTotal.toLocaleString()}</td>
              </tr>
              ...
              <tr>
               ...
               ${delivery > 0 ? `
              <tr>
                <td class="text-muted" style="text-align: right; padding: 3px 0; font-size: 14px;">Delivery:</td>
                <td class="text-main" style="text-align: right; font-weight: 600; padding: 3px 0; font-size: 14px;">+ Rs. ${delivery.toLocaleString()}</td>
              </tr>` : ''}

              <tr>
                <td class="text-main" style="text-align: right; padding: 10px 0 5px 0; font-weight: 700; font-size: 15px;">Grand Total:</td>
                <td class="text-main" style="text-align: right; padding: 10px 0 5px 0; font-weight: 700; font-size: 15px;">Rs. ${grandTotal.toLocaleString()}</td>
              </tr>

              <tr>
                <td class="text-muted" style="text-align: right; padding: 5px 0; border-top: 1px solid #f3f4f6;">Advance Paid:</td>
                <td class="text-main" style="text-align: right; font-weight: 600; padding: 5px 0; border-top: 1px solid #f3f4f6;">- Rs. ${advance.toLocaleString()}</td>
              </tr>
    ...
    await transporter.sendMail({
      from: '"ScrubX Orders" <orders@scrubx.com>',
      to: customerEmail,
      subject: `Receipt #${receiptId} - ScrubX`,
      html: emailHtml,
    });
